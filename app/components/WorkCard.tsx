...
export default function WorkCard({ work, index, featured = false, priority = false }: WorkCardProps) {
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-paper/12 bg-card transition-colors hover:border-copper/45">
      <div className={featured ? "lg:grid lg:grid-cols-[1.45fr_1fr]" : undefined}>
        <div
          className={
            featured
        ...
        <div className="flex flex-col p-5 sm:p-6 lg:justify-center lg:p-8">
          ...
          <p className="mt-3 text-sm leading-relaxed text-mute">{work.blurb}</p>
          <ul className="mt-4 flex flex-wrap gap-2">
            {work.tags.map((tag) => (
              <li
                key={tag}
                className="rounded-full border border-paper/15 px-2.5 py-1 font-mono text-[11px] text-mute"
              >
                {tag}
              </li>
            ))}
          ...
          <div className="mt-6 flex items-center gap-5 font-mono text-[11px] uppercase tracking-[0.16em]">
            <a
              ...
              className="text-paper underline decoration-copper/70 underline-offset-4 transition hover:decoration-copper"
            >
              Live
            </a>
            <a
